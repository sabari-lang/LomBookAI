...
const ReportFilters = ({
  ...
  const groupFieldsIntoRows = () => {
    ...
    // Special layout for Job Costing: keep From and To together in the same row
    if (reportId === 'jobCosting') {
      const rows = [];
      const fieldsPerRow = 4;

      // Reorder so that searchByDateType, fromDate, toDate appear sequentially
      const findByKey = (key) => visibleFields.find((f) => f.key === key);
      const searchBy = findByKey('searchByDateType');
      const fromField = findByKey('fromDate');
      const toField = findByKey('toDate');

      // Collect remaining fields excluding the three above
      const remaining = visibleFields.filter(
        (f) => !['searchByDateType', 'fromDate', 'toDate'].includes(f.key)
      );

      // First row: searchByDateType, fromDate, toDate, and one more if available
      const firstRow = [];
      if (searchBy) firstRow.push(searchBy);
      if (fromField) firstRow.push(fromField);
      if (toField) firstRow.push(toField);
      if (remaining.length > 0 && firstRow.length < fieldsPerRow) {
        firstRow.push(remaining.shift());
      }
      if (firstRow.length > 0) rows.push(firstRow);

      // Subsequent rows: chunk remaining as usual
      for (let i = 0; i < remaining.length; i += fieldsPerRow) {
        rows.push(remaining.slice(i, i + fieldsPerRow));
      }

      return rows;
    }

    // Default layout: 4 fields per row
    const rows = [];
    const fieldsPerRow = 4;
