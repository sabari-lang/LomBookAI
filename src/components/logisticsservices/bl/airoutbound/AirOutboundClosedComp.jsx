...
import { deleteAirOutboundJob, getAirOutboundClosedJobs } from "./airOutboundApi";
...
const AirOutboundClosedComp = () => {
    ...
    const deleteMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["airOutboundClosedJobs"] });
        },
    ...
    const { data: apiRaw, isLoading, isError } = useQuery({
        queryKey: ["airOutboundClosedJobs", currentPage, pageSize],
        queryFn: () =>
            getAirOutboundClosedJobs({
                page: currentPage,
                pageSize,
            }),
        ...
        <>
            <div className="container-fluid mt-3">
                <div className="d-flex justify-content-between align-items-center px-3 py-2 rounded-top" style={{ backgroundColor: "#0097A7", color: "white" }}>
                    <h5 className="m-0">Air Outbound - Closed</h5>
                </div>
                                ...
                                {!isLoading && !isError && rowsToRender.map((r) => (
                                    <tr key={r.id ?? r.masterNumber}>
                                        ...
                                        <td>
                                            ...
                                            <button className="btn btn-sm btn-primary" onClick={() => { sessionStorage.setItem("masterAirwayData", JSON.stringify(r)); navigate("/air-outbound/masterreport"); }}>
