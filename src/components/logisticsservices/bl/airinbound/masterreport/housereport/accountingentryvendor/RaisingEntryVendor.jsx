...
const RaisingEntryVendor = ({ editData, setEditData }) => {
    ...
    // ----------------------------------
    // AUTO CALC LOGIC - guarded to prevent unnecessary setValue
    // ----------------------------------
    useEffect(() => {
        ...
        arr.forEach((row, index) => {
            ...
            const prevAmountInInr = getValues(`${pathBase}.amountInInr`);
            const prevCgst = getValues(`${pathBase}.cgst`);
            const prevSgst = getValues(`${pathBase}.sgst`);
            const prevIgst = getValues(`${pathBase}.igst`);
            const prevTotal = getValues(`${pathBase}.total`);

            const nearlyEqual = (a, b) => Math.abs((Number(a) || 0) - (Number(b) || 0)) < 0.0001;

            if (!nearlyEqual(prevAmountInInr, amountInInr)) {
                setValue(`${pathBase}.amountInInr`, Number(amountInInr.toFixed(2)));
            }
            if (!nearlyEqual(prevCgst, cgst)) {
                setValue(`${pathBase}.cgst`, Number(cgst.toFixed(2)));
            }
            if (!nearlyEqual(prevSgst, sgst)) {
                setValue(`${pathBase}.sgst`, Number(sgst.toFixed(2)));
            }
            if (!nearlyEqual(prevIgst, igst)) {
                setValue(`${pathBase}.igst`, Number(igst.toFixed(2)));
            }
            if (!nearlyEqual(prevTotal, total)) {
                setValue(`${pathBase}.total`, Number(total.toFixed(2)));
            }
        });
    }, [watchedInputs, setValue, getValues]);
            ...
            <div className="modal-dialog modal-xl modal-dialog-scrollable">
                <div className="modal-content">
                    ...
                    <div className="modal-body">
                                ...
                                <table className="table table-bordered table-sm">
                                    <thead className="table-light">
                                        <tr>
                                            ...
                                            <th style={{ width: 200 }}>Units</th>
                                            <th style={{ width: 100 }}>CUR</th>
                                            <th style={{ width: 100 }}>SAC</th>
                        ...
                        {/* No global scoped CSS; widths are inline like CreateProvisionalEntry */}
                    </div>
                </div>
            </div>
        </div>
