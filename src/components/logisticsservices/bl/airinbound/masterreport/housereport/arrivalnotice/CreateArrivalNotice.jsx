...
const CreateArrivalNotice = ({ editData, setEditData }) => {
    ...
    const { data: provisionalApiRaw } = useQuery({
        ...
        enabled: Boolean(jobNo && hawb && !isEditing), // Only fetch when creating new (not editing)
        staleTime: 5 * 60 * 1000,
        retry: 1,
    });
    ...
    const autoFilledDefaults = useMemo(() => {
        return {
            ...
            branch: storedMaster?.branch ?? "HEAD OFFICE",
            ...
            shipperAddress: storedHouse?.shipperAddress ?? storedHouse?.shipper_address ?? "",
            ...
            consigneeAddress: storedHouse?.consigneeAddress ?? storedHouse?.consignee_address ?? "",
            // 7. Air Waybill (Not Negotiable Carrier) - Value Source: house.air_waybill or master.carrier_name
            airwbName: storedHouse?.airWaybill ?? storedHouse?.air_waybill ?? storedMaster?.carrierName ?? storedMaster?.carrier_name ?? "",
            airwbAddress: storedHouse?.airwbAddress ?? storedHouse?.airwb_address ?? "",
            // 8. Notify Name & Address - Value Source: house.notify_address
            notifyName: storedHouse?.notifyName ?? storedHouse?.notify_name ?? "",
            notifyAddress: storedHouse?.notifyAddress ?? storedHouse?.notify_address ?? "",
            ...
            origin: storedMaster?.airportDeparture ?? storedMaster?.airport_departure ?? "",
            // 10. Destination (Airport) - Value Source: master.airport_destination
            destination: storedMaster?.airportDestination ?? storedMaster?.airport_destination ?? "",
            ...
            departureDate: formatDateForInput(storedMaster?.departureDate ?? storedMaster?.departure_date),
            // 13. Arrival Date - Value Source: master.arrival_date
            arrivalDate: formatDateForInput(storedMaster?.arrivalDate ?? storedMaster?.arrival_date),

            // SECTION 4: PHYSICAL DETAILS FROM MASTER (editable, not readonly)
            // 14. No. of Pieces RCP - Value Source: master.no_pieces_rcp
            piecesRCP: storedMaster?.noPiecesRcp ?? storedMaster?.no_pieces_rcp ?? storedMaster?.pieces ?? "",
            // 15. Gross Weight - Value Source: master.gross_weight
            grossWeight: storedMaster?.grossWeight ?? storedMaster?.gross_weight ?? "",
            // 16. Chargeable Weight - Value Source: master.chargeable_weight
            chargeableWeight: storedMaster?.chargeableWeight ?? storedMaster?.chargeable_weight ?? "",
    ...
    // Auto-fill when modal is shown (Bootstrap modal event) - only for create mode
    useEffect(() => {
        const modalEl = document.getElementById("createArrivalNoticeModal");
        if (!modalEl) return;

        const handleModalShow = () => {
            // When modal opens in create mode, reset form with auto-filled values + mapped charges
            if (!isEditing && !initializedRef.current) {
                // Use current mappedCharges value at the time of modal open
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [isEditing]); // Only depend on isEditing, not on mappedCharges or autoFilledDefaults

    // Watch for changes in mappedCharges and update form immediately (for create mode only)
    useEffect(() => {
        if (!isEditing && mappedCharges.length > 0 && initializedRef.current) {
            // Update charges in form when provisional entries change
            setValue("charges", mappedCharges, { shouldDirty: false });
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [mappedCharges, isEditing]);
