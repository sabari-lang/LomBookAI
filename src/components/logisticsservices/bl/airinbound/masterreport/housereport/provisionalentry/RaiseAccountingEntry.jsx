// src/components/logisticsservices/bl/airinbound/masterreport/housereport/RaiseAccountingEntry.jsx
...
import {
    createAirInboundCustomerAccount,
    updateAirInboundCustomerAccount,
...
const PAGE_SIZE = 10; // default page size for modal table

const RaiseAccountingEntry = ({ editData }) => {
    const location = useLocation();
    const queryClient = useQueryClient();

    // Session values (explicitly only from sessionStorage)
    ...
    const isEditing = Boolean(editData?.id || editData?._id || location?.state?.id);
    ...
    const { data: provisionalApiRaw } = useQuery({
        queryKey: ["airInboundProvisionals", jobNo, hawbNo],
        queryFn: () => getAirInboundProvisionals(jobNo, hawbNo, { page: 1, pageSize: 1000 }),
    ...
    const provisionalList = useMemo(() => {
        ...
        return entries.flatMap((entry = {}) => {
            ...
            return items.map((it) => ({
                date: safeStr(it?.date ?? entry?.date ?? ""),
    ...
    // minimal, clean default values - auto-filled from master/house data
    const defaultValues = {
        voucherType: "",
        voucherNo: "",
        voucherDate: "",
    ...
    useEffect(() => {
        if (isEditing && editData) {
            // reset form with simple defaults overwritten by editData if available
            reset({
                ...defaultValues,
                ...editData,
                jobNo,
    ...
    // Pagination state (A2: Prev/Next only)
    ...
    // Normalize provisional rows to payload items
    ...
    // Mutations with handleProvisionalError on error
    ...
    const onSubmit = (form) => {
        const payload = {
            ...
            jobNo,
            hawbNo,
            // always send the full list (not only the current page)
                        ...
                        <form>
                            ...
                            <div className="row g-3 mt-2">
                                <div className="col-md-4">
                                    ...
                                    <Controller name="mobile" control={control} render={({ field }) => <input {...field} className="form-control" />} />
                                </div>
...
export default RaiseAccountingEntry;
