...
import { deletePEOceanInboundJob, getPEOceanInboundClosedJobs } from "../../../../../services/personal-effects/oceanInboundService";

const PEOceanInboundClosedComp = () => {
    ...
    const deleteMutation = useMutation({
        mutationFn: deletePEOceanInboundJob,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["peOceanInboundClosedJobs"] });
        },
    ...
    const handleDelete = async (row = {}) => {
        ...
        if (!jobNo) return;
        const confirmed = await confirm(`Delete Personal Effects Ocean Inbound job ${jobNo}?`);
        if (!confirmed) return;
    ...
    const { data: apiData, isLoading, isError } = useQuery({
        queryKey: ["peOceanInboundClosedJobs"],
        queryFn: getPEOceanInboundClosedJobs,
        retry: 1,
        staleTime: 1000 * 60 * 5,
    });
    ...
    return (
        <div className="container-fluid mt-3">
            <div className="d-flex justify-content-between align-items-center bg-secondary text-white px-3 py-2 rounded-top">
                <h5 className="m-0">Personal Effects - Ocean Inbound (Closed)</h5>
            </div>
                        ...
                        <tbody>
                            ...
                            {!isLoading && !isError && paginatedData?.map((row = {}) => (
                                <tr key={row.id ?? Math.random()}>
                                    ...
                                    <td>
                                        <button className="btn btn-sm btn-primary" onClick={() => { sessionStorage.setItem("masterAirwayData", JSON.stringify(row)); navigate("/pe/ocean-inbound/masterreport"); }}>
...
export default PEOceanInboundClosedComp;
