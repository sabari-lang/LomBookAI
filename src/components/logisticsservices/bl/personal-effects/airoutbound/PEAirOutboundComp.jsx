...
import { deletePEAirOutboundJob, getPEAirOutboundJobs } from "../../../../../services/personal-effects/airOutboundService";
import { notifyError } from "../../../../../utils/notifications";

const PEAirOutboundComp = () => {
    ...
    const deleteMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["peAirOutboundJobs"] });
        },
    ...
    const { data: apiData, isLoading, isError } = useQuery({
        queryKey: ["peAirOutboundJobs"],
        queryFn: getPEAirOutboundJobs,
        retry: 1,
        staleTime: 1000 * 60 * 5,
    });
    ...
    return (
        <div className="container-fluid mt-3">
            <div className="d-flex justify-content-between align-items-center bg-primary text-white px-3 py-2 rounded-top">
                <h5 className="m-0">Personal Effects - Air Outbound</h5>
                <button
                    className="btn btn-success btn-sm"
                    onClick={() => notifyError("PE Job Creation component needs to be implemented")}
                >
                            ...
                            {isLoading && (
                                <tr>
                                    <td colSpan="7" className="text-center py-4">
                                        <div className="spinner-border me-2 text-primary"></div>Loading...
                                    </td>
                                </tr>
                            )}
                            {isError && (
                                <tr>
                                    <td colSpan="7" className="text-center text-warning py-4">⚠ API Error — No Data</td>
                                </tr>
                            )}
                                ...
                                <tr key={row.id ?? Math.random()}>
                                    ...
                                    <td>
                                        <button
                                            ...
                                            onClick={() => {
                                                sessionStorage.setItem("masterAirwayData", JSON.stringify(row));
                                        ...
                                        <button
                                            ...
                                            disabled={deletingJobKey === row.jobNo && deleteMutation.isLoading}
...
export default PEAirOutboundComp;
