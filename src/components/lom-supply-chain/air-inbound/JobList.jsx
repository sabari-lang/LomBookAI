...
import { getLscAirInboundTracking, deleteLscAirInboundTracking } from "../../../services/lsc/airInbound/lscAirInboundTrackingService";
...
const JobList = ({ onEdit, onAdd }) => {
    ...
    const { data: apiRaw, isLoading, isError } = useQuery({
        queryKey: ["lscAirInboundTracking", currentPage, entriesPerPage, search],
        queryFn: () =>
            getLscAirInboundTracking({
                page: currentPage,
    ...
    const deleteMutation = useMutation({
        mutationFn: deleteLscAirInboundTracking,
        onSuccess: () => {
            queryClient.invalidateQueries(["lscAirInboundTracking"]);
            notifySuccess("Record deleted successfully!");
        },
        onError: (err) => handleProvisionalError(err, "Delete Air Inbound"),
        onSettled: () => {
            setDeletingId(null);
        },
    });
    ...
    const handleDelete = async (id) => {
        const confirmed = await confirm("Delete this Air Inbound record?");
        if (!confirmed) return;
        setDeletingId(id);
        ...
        <div className="container-fluid mt-3">
            ...
            <div className="d-flex justify-content-between align-items-center bg-primary text-white px-3 py-2 rounded-top">
                ...
                <button
                    ...
                    data-bs-target="#lscAirInboundJobModal"
                    onClick={onAdd}
                >
