...
import { getLscOceanOutboundTracking, deleteLscOceanOutboundTracking } from "../../../services/lsc/oceanOutbound/lscOceanOutboundTrackingService";
...
const JobList = ({ onEdit, onAdd }) => {
    ...
    const { data: apiRaw, isLoading, isError } = useQuery({
        queryKey: ["lscOceanOutboundTracking", currentPage, entriesPerPage, search],
        queryFn: () =>
            getLscOceanOutboundTracking({
                page: currentPage,
    ...
    const deleteMutation = useMutation({
        mutationFn: deleteLscOceanOutboundTracking,
        onSuccess: () => {
            queryClient.invalidateQueries(["lscOceanOutboundTracking"]);
            notifySuccess("Record deleted successfully!");
        },
        onError: (err) => handleProvisionalError(err, "Delete Ocean Outbound"),
        onSettled: () => {
            setDeletingId(null);
        },
    });
    ...
    const handleDelete = async (id) => {
        const confirmed = await confirm("Delete this Ocean Outbound record?");
        if (!confirmed) return;
        setDeletingId(id);
        ...
        <div className="container-fluid mt-3">
            <div className="d-flex justify-content-between align-items-center bg-primary text-white px-3 py-2 rounded-top">
                ...
                <button 
                    ...
                    data-bs-target="#lscOceanOutboundJobModal"
                    onClick={onAdd}
                >
