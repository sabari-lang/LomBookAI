...
import { getLscOceanInboundTracking, deleteLscOceanInboundTracking } from "../../../services/lsc/oceanInbound/lscOceanInboundTrackingService";
...
const JobList = ({ onEdit, onAdd }) => {
    ...
    const { data: apiRaw, isLoading, isError } = useQuery({
        queryKey: ["lscOceanInboundTracking", currentPage, entriesPerPage, search],
        queryFn: () =>
            getLscOceanInboundTracking({
                page: currentPage,
    ...
    const deleteMutation = useMutation({
        mutationFn: deleteLscOceanInboundTracking,
        onSuccess: () => {
            queryClient.invalidateQueries(["lscOceanInboundTracking"]);
            notifySuccess("Record deleted successfully!");
        },
        onError: (err) => handleProvisionalError(err, "Delete Ocean Inbound"),
        onSettled: () => {
            setDeletingId(null);
        },
    });
    ...
    const handleDelete = async (id) => {
        const confirmed = await confirm("Delete this Ocean Inbound record?");
        if (!confirmed) return;
        setDeletingId(id);
        ...
        <div className="container-fluid mt-3">
            <div className="d-flex justify-content-between align-items-center bg-primary text-white px-3 py-2 rounded-top">
                ...
                <button 
                    ...
                    data-bs-target="#lscOceanInboundJobModal"
                    onClick={onAdd}
                >
