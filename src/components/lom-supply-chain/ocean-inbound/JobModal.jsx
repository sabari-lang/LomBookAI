...
import { createLscOceanInboundTracking, updateLscOceanInboundTracking } from "../../../services/lsc/oceanInbound/lscOceanInboundTrackingService";
...
const JobModal = ({ editData, setEditData }) => {
    ...
    const handleCloseModal = () => {
        reset(DEFAULTS);
        setEditData?.(null);
        closeModalUtil("lscOceanInboundJobModal");
        cleanupModalBackdrop();
    };

    // Cleanup on modal close
    useEffect(() => {
        const modalElement = document.getElementById("lscOceanInboundJobModal");
    ...
    const createMutation = useMutation({
        mutationFn: createLscOceanInboundTracking,
        onSuccess: () => {
            queryClient.invalidateQueries(["lscOceanInboundTracking"]);
            notifySuccess("Ocean Inbound created successfully!");
            handleCloseModal();
        },
        onError: (err) => handleProvisionalError(err, "Create Ocean Inbound"),
    });

    const updateMutation = useMutation({
        mutationFn: ({ id, payload }) => updateLscOceanInboundTracking(id, payload),
        onSuccess: () => {
            queryClient.invalidateQueries(["lscOceanInboundTracking"]);
            notifySuccess("Ocean Inbound updated successfully!");
            handleCloseModal();
        },
        onError: (err) => handleProvisionalError(err, "Update Ocean Inbound"),
    });
            ...
            <div className="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
                <div className="modal-content">
                    <div className="modal-header">
                        <h5 className="modal-title fw-bold">Ocean Inbound</h5>
                        <button 
                            type="button" 
                            className="btn-close" 
