...
import { getTdsReceivable } from "../reportAPI";
...
const TDSReceivable = () => {
    ...
    const {
        ...
        () => getTdsReceivable({ fromDate, toDate, firm, page, perPage }),
        {
            keepPreviousData: true,
            onError: (err) => handleProvisionalError(err, "Fetch TDS Receivable", "Failed to fetch TDS Receivable report."),
        }
    );
    ...
    const columns = [
        ...
        { name: "INVOICE NO", selector: (r) => r.invoiceNo },
        {
            name: "TOTAL AMOUNT",
        ...
        {
            name: "TDS RECEIVABLE",
            selector: (r) => `₹ ${Number(r.tdsReceivable || 0).toLocaleString()}`,
            right: true,
        },
    ...
    const totalSaleWithTDS = filtered.reduce((a, b) => a + (b.totalAmount || 0), 0);
    const totalTDS = filtered.reduce((a, b) => a + (b.tdsReceivable || 0), 0);
    ...
    const generatePDF = async () => {
          ...
          <tbody>
            ${filtered.length
              ...
              <tr>
                ...
                <td>₹ ${(r.tdsReceivable || 0).toLocaleString()}</td>
                <td>${r.date || ""}</td>
                <td>${r.taxName || ""}</td>
                <td>${r.taxSection || ""}</td>
                <td>${r.tdsRate || ""}</td>
        ...
        <div class="flex">
          <div>Total Sale With TDS: ₹ ${totalSaleWithTDS.toLocaleString()}</div>
        ...
        return (
            <div className="container-fluid bg-light pb-4 overflow-auto rounded-3"
                ...
                <div className="d-flex justify-content-between align-items-center bg-white p-2 border rounded shadow-sm mb-2">
                    <h5 className="fw-bold mb-0">TDS Receivable</h5>
                ...
                <div className="d-flex justify-content-between px-2 mt-2 fw-bold">
                    <span className="text-success">Total Sale With TDS: ₹ {totalSaleWithTDS.toLocaleString()}</span>
