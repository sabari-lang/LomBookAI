...
@media print {
    ...
    #sales-order-print-sheet, #sales-order-print-sheet * {
...
const DownloadSalesOrder = ({ data: propData }) => {
    const { state: locationState } = useLocation();
    const printRef = useRef();

    // Use prop data or location state
    const rawData = propData || locationState || {};
    
    // Normalize data
    const data = {
        salesOrderNumber: rawData.salesOrderNumber || rawData.salesOrderNo || rawData.number || rawData.orderDetails?.number || "SO-00001",
        salesOrderDate: rawData.salesOrderDate || rawData.date || rawData.orderDetails?.date || "",
        referenceNumber: rawData.referenceNumber || rawData.reference || rawData.refNo || "",
        expectedShipmentDate: rawData.expectedShipmentDate || rawData.dueDate || rawData.orderDetails?.dueDate || "",
        customerName: rawData.customerName || rawData.customer?.name || "",
        customerAddress: rawData.customerAddress || rawData.customer?.address || rawData.billingAddress || "",
        customerGstin: rawData.customerGstin || rawData.customerGST || "",
        shippingAddress: rawData.shippingAddress || rawData.shipTo || "",
        shippingGstin: rawData.shippingGstin || rawData.shippingGST || "",
        placeOfSupply: rawData.placeOfSupply || "Tamil Nadu (33)",
        items: rawData.items || rawData.lineItems || [],
        subtotal: rawData.subtotal || rawData.subTotal || 0,
        taxAmount: rawData.taxAmount || rawData.tax || 0,
        totalAmount: rawData.totalAmount || rawData.total || 0,
        customerNotes: rawData.customerNotes || rawData.notes || "",
        terms: rawData.terms || rawData.termsAndConditions || ""
    };
    ...
    const taxInfo = parseTax(data.taxAmount || "18");
    
    const itemsWithTax = (data.items || []).map((item) => {
        const qty = Number(item.quantity || item.qty || 0);
        const rate = Number(item.rate || item.price || 0);
        const itemSubtotal = qty * rate;
        const itemTax = parseTax(item.tax || data.taxAmount || "18");
    ...
    const handleDownloadPDF = async () => {
        ...
        pdf.save(`SalesOrder_${data.salesOrderNumber || 'SO'}.pdf`);
    };
        ...
        <>
            ...
            <div className="container p-4" style={{ maxWidth: '900px', margin: '0 auto' }}>
                <div className="d-flex justify-content-end align-items-center gap-4 mb-3">
                    <i className="bi bi-download" style={{ fontSize: "22px", cursor: "pointer" }} title="Download PDF" onClick={handleDownloadPDF}></i>
                    <i className="bi bi-printer" style={{ fontSize: "22px", cursor: "pointer" }} title="Print" onClick={handlePrint}></i>
                </div>
                <div
                    ref={printRef}
                    id="sales-order-print-sheet"
                ...
                >
                    ...
                    <div style={headerStyle}>
                        ...
                        <div style={titleBlockStyle}>
                            <div style={documentTitleStyle}>SALES ORDER</div>
                            <div style={docDetailsStyle}>
                                <div style={docDetailRowStyle}>
                                    <span style={docLabelStyle}>Sales Order #:</span>
                                    <span style={docValueStyle}>{data.salesOrderNumber}</span>
                                </div>
                                <div style={docDetailRowStyle}>
                                    <span style={docLabelStyle}>Order Date:</span>
                                    <span style={docValueStyle}>{formatDate(data.salesOrderDate)}</span>
                                </div>
                                {data.referenceNumber && (
                                    <div style={docDetailRowStyle}>
                                        <span style={docLabelStyle}>Ref #:</span>
                                        <span style={docValueStyle}>{data.referenceNumber}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <div style={customerSectionStyle}>
                        <div style={billToBlockStyle}>
                            ...
                            <div style={customerNameStyle}>{data.customerName}</div>
                            {data.customerAddress && <div style={customerDetailStyle}>{data.customerAddress}</div>}
                            {data.customerGstin && <div style={customerDetailStyle}>GSTIN: {data.customerGstin}</div>}
                        </div>
                        <div style={shipToBlockStyle}>
                            <div style={sectionTitleStyle}>Ship To:</div>
                            {data.shippingAddress ? (
                                <>
                                    <div style={customerNameStyle}>{typeof data.shippingAddress === 'string' ? data.shippingAddress : data.shippingAddress.name || data.customerName}</div>
                                    {data.shippingGstin && <div style={customerDetailStyle}>GSTIN: {data.shippingGstin}</div>}
                                </>
                            ) : (
                                <>
                                    <div style={customerNameStyle}>{data.customerName}</div>
                    ...
                    <div style={signatureStyle}>
                        <div style={signatureBlockStyle}>
                            ...
                            <div style={signatureLabelStyle}>Authorized Signatory</div>
                        </div>
                    </div>
                </div>
            </div>
...
export default DownloadSalesOrder;
