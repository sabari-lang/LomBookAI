...
const DownloadInvoice = () => {
    ...
    if (!state || !Object.keys(state).length) {
        return (
            <div className="container p-4 text-center">
                ...
                <p className="text-muted">Please select an invoice from the list to view its report.</p>
            </div>
        );
    }
    ...
    const invoice = {
        invoiceNumber: data.invoiceNumber ?? data.invoiceNo ?? data.number ?? "",
        invoiceDate: data.invoiceDate ?? data.date ?? data.invoice_date ?? "",
        dueDate: data.dueDate ?? data.due_date ?? "",
        customerName: data.customerName ?? data.customer?.name ?? data.partyName ?? "",
        customerAddress: data.customerAddress ?? data.customer?.address ?? data.billingAddress ?? "",
        customerGstin: data.customerGstin ?? data.customerGST ?? "",
        shippingAddress: data.shippingAddress ?? data.shipTo ?? "",
        shippingGstin: data.shippingGstin ?? data.shippingGST ?? "",
        referenceNumber: data.referenceNumber ?? data.reference ?? data.ref ?? "",
        orderNumber: data.orderNumber ?? data.orderNo ?? data.order?.number ?? "",
        ...
        totalAmount: data.totalAmount ?? data.amount ?? data.total ?? 0,
        customerNotes: data.customerNotes ?? data.notes ?? "Thanks for your business.",
    ...
    const taxInfo = parseTax(invoice.taxAmount || "18");
    
    const itemsWithTax = (invoice.items || []).map((item) => {
        const qty = Number(item.quantity || 0);
        const rate = Number(item.rate || 0);
        const itemSubtotal = qty * rate;
        const itemTax = parseTax(item.tax || invoice.taxAmount || "18");
    ...
    const downloadPDF = async () => {
        ...
        pdf.save(`Invoice_${invoice.invoiceNumber || 'Invoice'}.pdf`);
    };
                ...
                >
                    ...
                    <div style={headerStyle}>
                        ...
                        <div style={titleBlockStyle}>
                            <div style={documentTitleStyle}>TAX INVOICE</div>
                            <div style={docDetailsStyle}>
                                <div style={docDetailRowStyle}>
                                    <span style={docLabelStyle}>Invoice Number:</span>
                                    <span style={docValueStyle}>{invoice.invoiceNumber || "INV-000005"}</span>
                                </div>
                                <div style={docDetailRowStyle}>
                                    <span style={docLabelStyle}>Invoice Date:</span>
                                    <span style={docValueStyle}>{formatDate(invoice.invoiceDate)}</span>
                                </div>
                                {invoice.dueDate && (
                                    <div style={docDetailRowStyle}>
                                        <span style={docLabelStyle}>Due Date:</span>
                                        <span style={docValueStyle}>{formatDate(invoice.dueDate)}</span>
                                    </div>
                                )}
                                {invoice.orderNumber && (
                                    <div style={docDetailRowStyle}>
                                        <span style={docLabelStyle}>P.O.N:</span>
                                        <span style={docValueStyle}>{invoice.orderNumber}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <div style={customerSectionStyle}>
                        <div style={billToBlockStyle}>
                            ...
                            {invoice.customerAddress && <div style={customerDetailStyle}>{invoice.customerAddress}</div>}
                            {invoice.customerGstin && <div style={customerDetailStyle}>GSTIN: {invoice.customerGstin}</div>}
                        </div>
                        <div style={shipToBlockStyle}>
                            <div style={sectionTitleStyle}>SHIP TO:</div>
                            {invoice.shippingAddress ? (
                                <>
                                    <div style={customerNameStyle}>{typeof invoice.shippingAddress === 'string' ? invoice.shippingAddress : invoice.shippingAddress.name || invoice.customerName}</div>
                                    {invoice.shippingGstin && <div style={customerDetailStyle}>GSTIN: {invoice.shippingGstin}</div>}
                                </>
                            ) : (
                                <>
                                    {invoice.customerGstin && <div style={customerDetailStyle}>GSTIN: {invoice.customerGstin}</div>}
