...
import { createDeposit } from "../api/dailyBankStatusApi";
...
const NewDepositModal = ({ modalId = "newDepositModal", onSaved }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            notifySuccess("Deposit saved");
            setForm(emptyForm);
            onSaved?.();
            closeModal(modalId);
        } catch (err) {
            notifyError(err?.message || "Failed to save deposit");
        } finally {
            setSaving(false);
            safeCleanupBackdrop();
        }
