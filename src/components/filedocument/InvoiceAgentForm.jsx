...
const InvoiceAgentForm = () => {
    ...
    // ✅ Watch line items in PascalCase - debounced to prevent lag
    const rawItems = useWatch({ control, name: "LineItems" }) || [];
    const items = useDebouncedValue(rawItems, 120);
    ...
    useEffect(() => {
        items?.forEach((row, i) => {
            ...
            // Guard: only setValue if value actually changed
            const currentCgst = getValues(`LineItems.${i}.Cgst`);
            const currentSgst = getValues(`LineItems.${i}.Sgst`);
            const currentIgst = getValues(`LineItems.${i}.Igst`);
    ...
    // 👉 Update form totals - guard to prevent unnecessary setValue calls
    useEffect(() => {
        const currentCurrency = getValues("Currency");
        const currentGst = getValues("Gst");
        const currentTotal = getValues("Total");
        
        const newCurrency = subtotal.toFixed(2);
        const newGst = taxTotal.toFixed(2);
        const newTotal = grandTotal.toFixed(2);
        
        const nearlyEqual = (a, b) => Math.abs((Number(a) || 0) - (Number(b) || 0)) < 0.01;
        
        if (!nearlyEqual(currentCurrency, newCurrency)) {
            setValue("Currency", newCurrency);
        }
        if (!nearlyEqual(currentGst, newGst)) {
            setValue("Gst", newGst);
        }
        if (!nearlyEqual(currentTotal, newTotal)) {
            setValue("Total", newTotal);
        }
    }, [subtotal, taxTotal, grandTotal, setValue, getValues]);
