// /mnt/data/CreateProvisionalEntry.jsx
...
import {
    createUbAirInboundProvisional,
    updateUbAirInboundProvisional,
...
const CreateProvisionalEntry = () => {
    ...
    const jobNo = safeStr(storedMaster?.jobNo) || safeStr(state?.jobNo);
    ...
    // Only observe input fields to avoid recursion
    ...
    // Auto-calc derived fields (depends only on inputs)
    useEffect(() => {
    ...
    // Immediate currency change handler
    const handleCurrencyChange = (index, newCurrency, fieldOnChange) => {
        ...
        // Recalculate immediately using watchedInputs snapshot
    ...
    const createMutation = useMutation({
        mutationFn: (payload) => createUbAirInboundProvisional(ubId, payload),
        onSuccess: () => {
            queryClient.invalidateQueries(["airInboundProvisionals", ubId]);
            queryClient.invalidateQueries(["airInboundProvisionals", jobNo, hawbNo]);
    ...
    const updateMutation = useMutation({
        mutationFn: (payload) => updateUbAirInboundProvisional(ubId, payload),
        onSuccess: () => {
            queryClient.invalidateQueries(["airInboundProvisionals", ubId]);
            queryClient.invalidateQueries(["airInboundProvisionals", jobNo, hawbNo]);
...
export default CreateProvisionalEntry;
