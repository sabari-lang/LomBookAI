...
import { getUbAirOutboundCustomerAccountById } from "../../../../../../services/personal-effects/airOutbound/peAirOutboundApi";
...
const ViewCustomerAccountOut = () => {
    ...
    useEffect(() => {
        const stored = sessionStorage.getItem("customerAccViewOut");
        if (stored) {
            try {
                const data = JSON.parse(stored);
                setEntryData(data);
            } catch (e) {
                console.error("Error parsing customerAccViewOut:", e);
            }
        }
    ...
    const { data: fullEntryData } = useQuery({
        queryKey: ["airOutboundCustomerAccountEntry", entryData?.id],
        queryFn: () => getAirOutboundCustomerAccountById(entryData?.id),
    ...
    const handlePrintExportInvoice = () => {
        ...
        const leftColumnData = [
            ...
            ["Master No", masterData?.mawbNo || masterData?.mawb || ""],
            ["House No", displayData?.hawbNo || displayData?.hawbno || houseData?.hawb || houseData?.hawbNo || ""],
            ["Shipper", masterData?.shipperName || ""],
            ["Flight No", masterData?.flightNo || ""],
            ["Origin", masterData?.airportDeparture || ""],
            ["Destination", masterData?.airportDestination || ""],
            ["Gross Weight", `${masterData?.grossWeight || ""} ${masterData?.kgLb || "KG"}`],
            ["Chargable Weight", `${masterData?.chargeableWeight || ""} ${masterData?.kgLb || "KG"}`],
            ["No of Pieces", masterData?.pieces || ""],
        ...
        const rightColumnData = [
            ...
            ["ETD", formatDate(masterData?.departureDate)],
        ];
    ...
    const handlePrintInvoice = async () => {
        const element = document.getElementById('invoice-print-content-airout');
        if (!element) return;
    ...
    return (
        <div className="container-fluid p-3" style={{ backgroundColor: "#f5f5f5", minHeight: "100vh" }} ref={printRef}>
            ...
            `}</style>
            {/* Hidden Invoice Content for PDF Generation (AIR OUT) */}
            <div id="invoice-print-content-airout" style={{ 
                position: 'absolute', 
                left: '-9999px', 
                top: 0, 
                width: '210mm', 
                ...
                <table style={{ width: '100%', marginBottom: '8px', border: '1px solid #000', borderCollapse: 'collapse', fontSize: '9px' }}>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style={{ border: '1px solid #000', padding: '4px' }}>{grossWeight} {masterData?.kgLb || 'KG'}</td>
                        </tr>
                        <tr>
                            ...
                            <td style={{ border: '1px solid #000', padding: '4px' }}>{chargeableWeight} {masterData?.kgLb || 'KG'}</td>
                        </tr>
                        <tr>
                ...
                <table style={{ width: '100%', marginBottom: '8px', border: '1px solid #000', borderCollapse: 'collapse', fontSize: '9px' }}>
                    <thead>
                        <tr>
                            <th style={{ border: '1px solid #000', padding: '5px', textAlign: 'left', width: '12%', backgroundColor: '#f5f5f5', fontWeight: 'bold' }}>Code</th>
                            <th style={{ border: '1px solid #000', padding: '5px', textAlign: 'left', backgroundColor: '#f5f5f5', fontWeight: 'bold' }}>Description</th>
                            <th style={{ border: '1px solid #000', padding: '5px', textAlign: 'center', width: '15%', backgroundColor: '#f5f5f5', fontWeight: 'bold' }}>SAC</th>
                            <th style={{ border: '1px solid #000', padding: '5px', textAlign: 'right', width: '18%', backgroundColor: '#f5f5f5', fontWeight: 'bold' }}>Debit</th>
                        </tr>
                    </thead>
                    <tbody>
                        {items.length === 0 ? (
...
export default ViewCustomerAccountOut;
