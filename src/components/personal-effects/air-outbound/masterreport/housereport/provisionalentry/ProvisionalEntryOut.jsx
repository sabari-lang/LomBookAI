...
import RaiseAccountingEntryOut from "./RaiseAccountingEntryOut";
import AccountingEntrCusOut from "../accountingentrycus/AccountingEntryCusOut";
import AccountingEntryDetailedOut from "../accountingentrycustomerdetail/AccountingEntryCusDetailedOut";
import AccountingEntryVendorOut from "../accountingentryvendor/AccountingEntryVendorOut";
import AccountingEntryVendorDetailedOut from "../accountingentryvendordetail/AccountingEntryVendorDetailedOut";
import JobCostingOut from "../jobcosting/JobcostingOut";
import CreateHouseAirwayBillOut from "../../masterairwaybill/CreateHouseAirwayBillOut";
...
import {
    deleteUbAirOutboundProvisional,
    getUbAirOutboundProvisionals,
} from "../../../../../../services/personal-effects/airOutbound/peAirOutboundApi";

const ProvisionalEntryOut = () => {
    ...
    const { data: apiRaw, isLoading } = useQuery({
        queryKey: ["airOutboundProvisionals", jobNo, hawb, currentPage, entriesPerPage],
        queryFn: () =>
            getAirOutboundProvisionals(jobNo, hawb, {
                page: currentPage,
    ...
    //========================
    // NORMALIZE EXACT MATCH WITH INBOUND
    //========================
    ...
    //========================
    // FIXED PAGINATION + SEARCH (same as inbound)
    //========================
    ...
    const deleteMutation = useMutation({
        mutationFn: () => deleteAirOutboundProvisional(jobNo, hawb),
        onSuccess: () => {
            notifySuccess("Provisional Entry Deleted");
            queryClient.invalidateQueries(["airOutboundProvisionals", jobNo, hawb]);
        },
        onError: (error) => {
    ...
    const total = rowsToRender.reduce((sum, r) => sum + Number(r.total ?? 0), 0);
    ...
    return (
        <>
            <div className="container-fluid tw-mt-4">
                ...
                {!masterCollapsed && <CreateHouseAirwayBillOut />}
                ...
                <CommonSectionHeader
                    ...
                    onButtonClick={() =>
                        navigate("/pe/air-outbound/masterreport/housereport/create-provisional")
                    }
                                            ...
                                            <tr key={row.id + idx}>
                                                ...
                                                <td className="text-center">
                                                    <button
                                                        ...
                                                        onClick={() =>
                                                            navigate(
                                                                "/pe/air-outbound/masterreport/housereport/create-provisional",
                                                                { state: row.__raw }
                                                            )
                                                        }
                                                    >
                ...
                {/* MODALS */}
                <AccountingEntrCusOut />
                <AccountingEntryDetailedOut />
                <AccountingEntryVendorOut />
                <AccountingEntryVendorDetailedOut />
                <JobCostingOut />
            </div>

            <RaiseAccountingEntryOut />
        </>
    );
};

export default ProvisionalEntryOut;
