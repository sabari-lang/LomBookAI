...
import {
    deleteUbOceanOutboundJob,
    getUbOceanOutboundJobs,
} from "../../../services/personal-effects/oceanOutbound/peOceanOutboundApi"; // ← API like AirOutbound
...
const OceanOutboundComp = () => {
    ...
    const deleteMutation = useMutation({
        mutationFn: deleteUbOceanOutboundJob,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["pe-ub-ocean-outbound-master"] });
        },
    ...
    const handleDelete = async (row = {}) => {
        ...
        if (!jobNo) return;
        const confirmed = await confirm(`Delete UB Ocean Outbound job ${jobNo}?`);
    if (!confirmed) return;
    ...
    const { data: apiData, isLoading, isError } = useQuery({
        queryKey: ["pe-ub-ocean-outbound-master"],
        queryFn: getUbOceanOutboundJobs,
        retry: 1,
        staleTime: 1000 * 60 * 5,
    });
        ...
        <>
            <div className="container-fluid mt-3">
                ...
                <div className="d-flex justify-content-between align-items-center bg-primary text-white px-3 py-2 rounded-top">
                    <h5 className="m-0">UB Ocean Outbound</h5>

                    <button
                        className="btn btn-success btn-sm"
                        data-bs-toggle="modal"
                        data-bs-target="#peOceanOutCreateJobModal"
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        sessionStorage.setItem(
                                                            "peUbMasterBillOfLaddingData",
                                                            JSON.stringify(row ?? {})
                                                        );
                            navigate("/pe/ocean-outbound/masterreport");
                                                    }}
                                                >
