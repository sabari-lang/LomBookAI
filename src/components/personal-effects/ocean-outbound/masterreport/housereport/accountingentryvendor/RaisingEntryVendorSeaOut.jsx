...
import { createUbOceanOutboundVendorAccount, updateUbOceanOutboundVendorAccount } from "../../../../../../services/personal-effects/oceanOutbound/peOceanOutboundApi";
...
const RaisingEntryVendorSeaOut = ({ editData, setEditData }) => {
  ...
  const storedMaster = JSON.parse(
    sessionStorage.getItem("peUbMasterBillOfLaddingData") || "{}"
  );

  console.log("storedMaster", storedMaster);
  const storedHouse = JSON.parse(
    sessionStorage.getItem("peUbHouseBillOfLaddingData") || "{}"
  );
  ...
  const ubId = storedHouse?.id ?? storedHouse?._id ?? storedHouse?.ubId ?? "";
  ...
  // Currency Change
  // ---------------------------------------------
  const handleCurrencyChange = (index, currency, onChange) => {
    const exRate = safeNum(exchangeRates[currency] ?? 1);
    onChange(currency);
    setValue(`items.${index}.exRate`, exRate);
  ...
  const createMutation = useMutation({
    mutationFn: ({ ubId, payload }) =>
      createUbOceanOutboundVendorAccount(ubId, payload),

    onError: (err) =>
      handleProvisionalError(err, "Create UB Ocean Outbound Vendor Accounting"),
    ...
    onSuccess: () => {
      ...
      const modalElement = document.getElementById("seaoutraiseVendorModalOut");
      if (modalElement) {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ ubId, payload }) =>
      updateUbOceanOutboundVendorAccount(ubId, payload),

    onError: (err) =>
      handleProvisionalError(err, "Update UB Ocean Outbound Vendor Accounting"),
  ...
  const onSubmit = (data) => {
    ...
    if (isEditing) {
      updateMutation.mutate({ ubId, payload });
    } else {
      createMutation.mutate({ ubId, payload });
    }
  };
          ...
          <div className="modal-header">
            ...
            <button
              ...
              onClick={() => {
                reset(defaultValues);
                setEditData(null);
                closeModalById("seaoutraiseVendorModalOut");
              }}
            ></button>
          </div>
...
export default RaisingEntryVendorSeaOut;
