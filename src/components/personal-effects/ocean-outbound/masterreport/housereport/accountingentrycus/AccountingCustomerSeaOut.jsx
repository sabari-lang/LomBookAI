...
import {
    getUbOceanOutboundCustomerAccounts,
    deleteUbOceanOutboundCustomerAccount
...
const AccountingCustomerSeaOut = () => {
    ...
    const { data: apiRaw, isLoading, isError } = useQuery({
        queryKey: ["oceanOutboundCustomerAcc", ubId, currentPage, pageSize, search],
        queryFn: () =>
            getUbOceanOutboundCustomerAccounts(ubId, {
                page: currentPage,
                pageSize,
                search,
            }),
    ...
    const deleteMutation = useMutation({
        mutationFn: () => deleteUbOceanOutboundCustomerAccount(ubId),
        onSuccess: () => {
            queryClient.invalidateQueries(["oceanOutboundCustomerAcc"]);
            notifySuccess("Deleted Successfully");
        },
        onError: (err) =>
            handleProvisionalError(err, "Delete UB Ocean Outbound Customer Accounting"),
    });

    const handleDelete = async (row) => {
        if (!ubId) return notifyError("Missing Shipment ID!");
        const confirmed = await confirm("Delete all accounting entries for this shipment?");
    if (!confirmed) return;
    ...
    const handleView = (row) => {
        ...
        navigate("/pe/ocean-outbound/masterreport/housereport/view-customer-accounting");
    };
