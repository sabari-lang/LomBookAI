...
import { createUbOceanOutboundJob, updateUbOceanOutboundJob } from "../../../services/personal-effects/oceanOutbound/peOceanOutboundApi";
...
const JobCreation = ({ editData, setEditData }) => {
  ...
  const handleCloseModal = () => {
    reset(initialValues);
    setEditData?.(null);
    closeModalUtil("peOceanOutCreateJobModal");
    cleanupModalBackdrop();
  };

  // POST API 
  const createMutation = useMutation({
    mutationFn: createUbOceanOutboundJob,
    onSuccess: () => {
      queryClient.invalidateQueries(["pe-ub-ocean-outbound-master"]);
      notifySuccess("UB Job Created Successfully");
      handleCloseModal();
    },
    onError: (error) => {
      const message =
      ...
      handleProvisionalError(error, "Create UB Job");
    },
  });

  // PUT API
  const updateMutation = useMutation({
    mutationFn: ({ id, payload }) => updateUbOceanOutboundJob(id, payload),
    onSuccess: () => {
      queryClient.invalidateQueries(["pe-ub-ocean-outbound-master"]);
      notifySuccess("UB Job Updated Successfully");
      handleCloseModal();
    },
    onError: (error) => {
      const message =
      ...
      handleProvisionalError(error, "Update UB Job");
    },
  });
  ...
  useEffect(() => {
    const modalElement = document.getElementById("peOceanOutCreateJobModal");
  ...
  return (
    <>
      <div className="modal fade" id="peOceanOutCreateJobModal" tabIndex="-1" aria-hidden="true" data-bs-backdrop="static">
        <div className="modal-dialog modal-xl modal-dialog-centered modal-fullscreen-lg-down">
          <div className="modal-content" style={{ borderRadius: 6 }}>
            <div className="modal-header">
              <h4 className="fw-bold m-0">{editData?.id ? "Edit UB Job Creation" : "UB Job Creation"}</h4>
              <button
                type="button"
                className="btn-close"
