...
import { createOceanInboundVendorAccount, updateOceanInboundVendorAccount } from "../../../../../../services/personal-effects/oceanInbound/peOceanInboundApi";
...
const RaisingEntryVendorSeaIn = ({ editData, setEditData }) => {
    ...
    const defaultValues = {
        ...
        refDate: "",
        mawbno: mawbNo,
        hawbno: hblNo, // field name kept same as UI (H.B/L No)
    ...
    // auto calculation (amountInInr, cgst, sgst, igst, total)
    useEffect(() => {
        ...
        // stringify ensures deep compare and avoids unnecessary loops
    }, [JSON.stringify(watchedInputs), setValue]);

    // currency change handler (uses exchangeRates)
    ...
    // edit-mode load (safe fallback + date formatting)
    useEffect(() => {
        if (!isEditing) return;
    ...
    // totals
    const subtotal = watchedItems
        .reduce((sum, i) => sum + safeNum(i.amountInInr), 0)
        .toFixed(2);
    const grandTotal = watchedItems
    ...
    const createMutation = useMutation({
        mutationFn: ({ jobNo, hblNo, payload }) =>
            createOceanInboundVendorAccount(jobNo, hblNo, payload),
        onError: (err) => handleProvisionalError(err, "Create UB Ocean Inbound Vendor"),
        onSuccess: () => {
            ...
            const modalElement = document.getElementById("raiseVendorSeaInModal");
            if (modalElement) {
    ...
    const updateMutation = useMutation({
        mutationFn: ({ jobNo, hblNo, payload }) =>
            updateOceanInboundVendorAccount(jobNo, hblNo, payload),
        onError: (err) => handleProvisionalError(err, "Update UB Ocean Inbound Vendor"),
        onSuccess: () => {
            queryClient.invalidateQueries(["oceanInboundVendor"]);
    ...
    const onSubmit = (data) => {
        ...
        if (isEditing) {
            updateMutation.mutate({ jobNo, hblNo: hblNo, payload });
        } else {
            createMutation.mutate({ jobNo, hblNo: hblNo, payload });
        }
    };
                            ...
                            <form onSubmit={handleSubmit(onSubmit)}>
                                ...
                                <div className="row g-3">
                                    ...
                                    <div className="col-md-3">
                                        ...
                                        <Controller name="baseCurrency" control={control} render={({ field }) => (
                                            <select {...field} className="form-select">
                                    ...
                                    {/* M.B/L No / H.B/L No / Status / JobNo (readonly) */}
                                    <div className="col-md-3 mt-2">
...
export default RaisingEntryVendorSeaIn;
