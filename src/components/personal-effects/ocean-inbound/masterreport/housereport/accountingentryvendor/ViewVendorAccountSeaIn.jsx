...
import { getOceanInboundVendorAccountById } from "../../../../../../services/personal-effects/oceanInbound/peOceanInboundApi";
...
const ViewVendorAccountSeaIn = () => {
    ...
    useEffect(() => {
        const stored = sessionStorage.getItem("vendorSeaInView");
        if (stored) {
            try {
                const data = JSON.parse(stored);
                setEntryData(data);
            } catch (e) {
                console.error("Error parsing vendorSeaInView:", e);
            }
        }
    ...
    const { data: fullEntryData } = useQuery({
        queryKey: ["oceanInboundVendorAccountEntry", entryData?.id],
        queryFn: () => getOceanInboundVendorAccountById(entryData?.id),
    ...
    const handlePrintInvoice = async () => {
        ...
        try {
            ...
            // Build HTML for Purchase Voucher (matching uploaded image)
            tempDiv.innerHTML = `
                <div style="display:flex; align-items:flex-start; justify-content:space-between; margin-bottom:15px;">
                    <div style="display:flex; align-items:center; gap:10px;">
                        <img src="${images.lomlogo}" alt="LOM" style="height:40px; object-fit:contain;"/>
                        <div style="font-size:10px; color:#FF0000;">We Care, We Monitor, We Deliver</div>
                    </div>
                    <div style="text-align:right; max-width:55%;">
                        <div style="font-size:16px; font-weight:bold; margin-bottom:5px; color:#0000FF;">LOM LOGISTICS INDIA PVT.LTD.</div>
                ...
                <div style="text-align:center; font-size:16px; font-weight:bold; margin:10px 0 12px 0;">Purchase</div>

                <div style="margin-bottom:12px; font-size:11px;">
                    <table style="width:100%; border-collapse:collapse; font-size:11px;">
                        <tbody>
                            <tr>
                                <td style="width:20%; padding:3px; font-weight:bold;">Voucher No :</td>
                                <td style="padding:3px;">${voucherNo}</td>
                                <td style="width:20%; padding:3px; font-weight:bold; text-align:right;">Rs</td>
                                <td style="width:15%; padding:3px; text-align:right;">${firstItem ? Number(firstItem?.total || 0).toFixed(2) : Number(totalAmount||0).toFixed(2)}</td>
                            </tr>
                            <tr>
                                <td style="padding:3px; font-weight:bold;">Voucher Date :</td>
                                <td style="padding:3px;">${voucherDate}</td>
                                <td style="padding:3px; font-weight:bold; text-align:right;">IGST Rs</td>
                                <td style="padding:3px; text-align:right;">${firstItem ? Number(firstItem?.igst || 0).toFixed(2) : Number(igstTotal||0).toFixed(2)}</td>
                            </tr>
                            <tr>
                                <td style="padding:3px; font-weight:bold;">Credit Account :</td>
                                <td style="padding:3px;">${partyName}</td>
                            ...
                            <tr>
                                ...
                                <td style="padding:3px;">${partyAddress}</td>
                                <td style="padding:3px; font-weight:bold; text-align:right;">&nbsp;</td>
                                <td style="padding:3px; text-align:right;">&nbsp;</td>
                            </tr>
                            <tr>
                                <td style="padding:3px; font-weight:bold;">GSTIN :</td>
                                <td style="padding:3px;">${gstin}</td>
                                <td style="padding:3px; font-weight:bold; text-align:right;">&nbsp;</td>
                                <td style="padding:3px; text-align:right;">&nbsp;</td>
                            </tr>
                            ${firstItem ? `
                            <tr>
                                <td style="padding:3px; font-weight:bold;">Debit Account :</td>
                                <td style="padding:3px;">${firstItem?.description || ""}</td>
                                <td style="padding:3px; text-align:right;">&nbsp;</td>
                                <td style="padding:3px; text-align:right;">&nbsp;</td>
                            </tr>
                            ` : ""}
                    ...
                    <table style="width: 100%; border-collapse: collapse; font-size: 10px; margin-bottom: 10px;">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td style="border: 1px solid #000; padding: 5px;">${mblNo}</td>
                                <td style="border: 1px solid #000; padding: 5px;">${hblNo}</td>
    ...
    return (
        <div className="container-fluid p-3" style={{ backgroundColor: "#f5f5f5", minHeight: "100vh" }} ref={printRef}>
            ...
            <div className="p-4 accounting-entry-print" style={{ backgroundColor: "transparent", maxWidth: "1200px", margin: "0 auto" }}>
                ...
                {/* Itemized Charges Table with all columns */}
                <div className="mb-4">
                            ...
                            {items.length === 0 ? (
                                ...
                                items.map((item, index) => (
                                    <tr key={index}>
                                        ...
                                        <td style={{ padding: "8px" }}>{item?.currency || item?.cur || "INR"}</td>
                                        <td style={{ padding: "8px" }}>{item?.per || item?.units || ""}</td>
            ...
            {/* PDF Preview Dialog */}
            {showPreview && (
                <div className="d-print-none" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ background: 'white', width: '80vw', height: '80vh', borderRadius: 6, boxShadow: '0 8px 24px rgba(0,0,0,0.2)', display: 'flex', flexDirection: 'column' }}>
                        <div className="d-flex justify-content-between align-items-center p-2 border-bottom">
                            <div className="fw-bold">Invoice Preview</div>
                            <div className="d-flex gap-2">
                                <button className="btn btn-sm btn-secondary" onClick={() => {
                                    try {
                                        const fileName = `Purchase_Voucher_${displayData?.voucherNo || displayData?.id || 'voucher'}.pdf`;
                                ...
                                <button className="btn btn-sm btn-primary" onClick={() => {
                                    try {
                                        if (pdfUrl) {
                                            ...
                                            if (w) {
                                                // printing a blob URL PDF may require user action
                                            }
                                        }
                                    } catch {}
                                }}>Print</button>
                        ...
                        <div style={{ flex: 1, overflow: 'auto' }}>
                            {pdfUrl ? (
                                <iframe title="PDF Preview" src={pdfUrl} style={{ width: '100%', height: '100%', border: 0 }}></iframe>
                            ) : (
...
export default ViewVendorAccountSeaIn;
