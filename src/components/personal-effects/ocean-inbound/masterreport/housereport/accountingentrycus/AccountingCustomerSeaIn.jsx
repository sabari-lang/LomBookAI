...
import {
    ...
    deleteOceanInboundCustomerAccount
} from "../../../../../../services/personal-effects/oceanInbound/peOceanInboundApi";

import RaiseAccountingEntrySeaIn from "../provisionalentry/RaiseAccountingEntrySeaIn";
import { notifySuccess, notifyError, notifyInfo } from "../../../../../../utils/notifications";
import { confirm } from "../../../../../../utils/confirm";

const AccountingCustomerSeaIn = () => {
    ...
    const { data: apiRaw, isLoading, isError } = useQuery({
        queryKey: ["oceanInboundCustomerAcc", jobNo, hblNo, currentPage, pageSize, search],
        queryFn: () =>
            getOceanInboundCustomerAccounts(jobNo, hblNo, {
                page: currentPage,
                pageSize,
                search,
            }),
    ...
    // -------------------------------------
    // RAW ENTRIES MAP (for edit modal)
    // -------------------------------------
    ...
    // -------------------------------------
    // NORMALIZE EACH ENTRY LIKE PROVISIONAL
    // -------------------------------------
    ...
    // -------------------------------------
    // PAGINATION (same provisional fix)
    // -------------------------------------
    const pagination = extractPagination(apiRaw ?? {});
    const backendHas = pagination?.totalPages > 0 && pagination?.totalCount > 0;

    const totalPages = backendHas
        ? pagination.totalPages
        : Math.max(1, Math.ceil(filtered.length / pageSize));

    const totalCount = backendHas
    ...
    const deleteMutation = useMutation({
        mutationFn: ({ id }) => deleteOceanInboundCustomerAccount(id),
        onSuccess: () => {
            queryClient.invalidateQueries(["oceanInboundCustomerAcc"]);
            notifySuccess("Deleted Successfully");
        },
        onError: (err) =>
            handleProvisionalError(err, "Delete UB Ocean Inbound Customer Accounting"),
    });
    ...
    const handleView = (row) => {
        sessionStorage.setItem("customerAccViewSeaIn", JSON.stringify(row));
        navigate("/pe/ocean-inbound/masterreport/housereport/view-customer-accounting");
    };
    ...
    const handleEdit = (row) => {
        ...
        const el = document.getElementById("SeaInRaiseAccountingModal");
        if (el) {
    ...
    return (
        <>
            ...
            {/* MODAL */}
            <RaiseAccountingEntrySeaIn editData={editData} setEditData={setEditData} />
        </>
    );
};

export default AccountingCustomerSeaIn;
