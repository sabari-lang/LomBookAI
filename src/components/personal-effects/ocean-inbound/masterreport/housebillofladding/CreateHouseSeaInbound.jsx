...
import { createOceanInboundHouse, updateOceanInboundHouse } from "../../../../../services/personal-effects/oceanInbound/peOceanInboundApi";
...
const CreateHouseSeaInbound = ({ editData, setEditData }) => {
  ...
  const initialValues = {
    // TOP / header / basic - Auto-filled from job/master
    ...
    // dates & terms - Auto-filled from job/master
  ...
  const closeModal = () => {
    ...
    const modalElement = document.getElementById("seainboundCreateHouseModal");
    if (modalElement) {
  ...
  // POST API - UB Ocean Inbound
  const createOceanInboundHouseMutation = useMutation({
    mutationFn: ({ jobNo, payload }) =>
      createOceanInboundHouse(jobNo, payload),

    onSuccess: () => {
      queryClient.invalidateQueries(["peUbOceanInboundHouses", jobNo]);
      notifySuccess("UB Ocean Inbound House Created Successfully");
      closeModal();
    },
  ...
  // PUT API - UB Ocean Inbound
  const updateOceanInboundHouseMutation = useMutation({
    mutationFn: ({ jobNo, hblNo, payload }) =>
      updateOceanInboundHouse(jobNo, hblNo, payload),

    onSuccess: (response) => {
      queryClient.invalidateQueries(["peUbOceanInboundHouses", jobNo]);
      // Store house data after successful update
      const updatedData = response?.data || response || { ...editData, ...getValues() };
      if (updatedData) {
        sessionStorage.setItem("peUbHouseBillOfLaddingData", JSON.stringify(updatedData));
      }
      notifySuccess("UB Ocean Inbound House Updated Successfully");
      closeModal();
    },
  ...
  const onSubmit = (form) => {
    ...
    if (isEditing) {
      updateOceanInboundHouseMutation.mutate({
        jobNo: editData?.jobNo,
        hblNo: editData?.hblNo, // HBL number
        payload: data,
      });
    } else {
      createOceanInboundHouseMutation.mutate({
        jobNo,
        payload: data,
      });
    }
              ...
              <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="modal-footer mt-3">
                  ...
                  <button
                    ...
                    disabled={createOceanInboundHouseMutation.isLoading || updateOceanInboundHouseMutation.isLoading}
                  >
                    {(createOceanInboundHouseMutation.isLoading || updateOceanInboundHouseMutation.isLoading) ? (
                      <>
...
export default CreateHouseSeaInbound;
