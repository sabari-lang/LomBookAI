...
const PurchaseOrders = () => {
  ...
  const generatePDF = async () => {
    ...
    div.innerHTML = `
      <h2 style="text-align:center;margin-bottom:10px;">PURCHASE ORDER REPORT</h2>
      <table style="width:100%;border-collapse:collapse;font-size:12px;">
        <thead>
          <tr>
            <th style="border:1px solid #000;padding:6px;">Date</th>
            <th style="border:1px solid #000;padding:6px;">PO Number</th>
            ...
            .map(
              (row) => `
              <tr>
                <td style="border:1px solid #000;padding:6px;">${formatDate(row?.date ?? row?.createdAt)}</td>
                <td style="border:1px solid #000;padding:6px;">${row?.purchaseOrderNo ?? row?.poNumber ?? ""}</td>
                <td style="border:1px solid #000;padding:6px;">${row?.vendorName ?? (row?.vendor?.name ?? "")}</td>
                <td style="border:1px solid #000;padding:6px;">${row?.status ?? ""}</td>
                <td style="border:1px solid #000;padding:6px;">${formatAmount(row?.amount ?? row?.total ?? 0)}</td>
              </tr>`
            )
            .join("")}
            </tbody>
  ...
  const columns = [
    { name: "Date", selector: (row) => formatDate(row?.date ?? row?.createdAt), sortable: true },
    { name: "PO Number", selector: (row) => row?.purchaseOrderNo ?? row?.poNumber ?? "", sortable: true },
    { name: "Vendor", selector: (row) => row?.vendorName ?? (row?.vendor?.name ?? ""), sortable: true },
    {
      name: "Status",
      cell: (row) => (
        <span
          ...
          <button
            ...
            onClick={(e) => {
              ...
              try {
                ...
                if (!id) throw new Error("PO id missing");
                if (!original?.vendorName && !original?.vendor?.name) throw new Error("Vendor required to convert to bill");
                if (!Array.isArray(original?.items) || original.items.length === 0) throw new Error("PO has no items to convert");

                console.info(`[PO -> Bill] Converting PO ${original?.purchaseOrderNo ?? id} to Bill`);
                ...
                navigate("/newbill", {
                  state: {
                    ...
                    sourcePurchaseOrderNumber: original?.purchaseOrderNo ?? original?.poNumber ?? null,
                    sourceType: "purchaseorder",
                    conversionTimestamp: new Date().toISOString(),
                  },
                });
              } catch (err) {
                console.error("Convert PO -> Bill error:", err);
                handleProvisionalError(err, "Convert to Bill", err?.message || "Failed to convert Purchase Order to Bill");
              }
            }}
          >
            Convert
